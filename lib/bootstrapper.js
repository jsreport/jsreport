...
module.exports = function (options) {
    ...
    if (!fs.existsSync(path.join(options.rootDirectory, getConfigFile()))) {
        var pathToExampleConfig = options.pathToExampleConfig || path.join(options.rootDirectory, "example.config.json");
    ...
    nconf
        .argv()                     //  argv is fourth
        .env()                      //  env is third
        .file({ file: path.join(options.rootDirectory, getConfigFile()) }) //  file is second
        .defaults(options);
...
Bootstrapper.prototype.start = function () {
    ...
    return q().then(function () {
        ...
        if (!nconf.get('cluster') || nconf.get("cluster").enabled === false) {
        ...
        if (self._cluster.isMaster) {
            ...
            var numberOfWorkers = nconf.get("cluster").numberOfWorkers || numCPUs;
...
Bootstrapper.prototype._initWinston = function () {
    if (!winston.loggers.has("jsreport")) {
        ...
        var fileTransport = new (winston.transports.File)({ name: "main", filename: 'logs/reporter.log', maxsize: 10485760, json: false, level: transportSettings.level });
...
Bootstrapper.prototype._initHttpServer = function () {
    ...
    if (!nconf.get('httpPort') && !nconf.get('httpsPort')) {
        return q.ninvoke(this._app, "listen", process.env.PORT);
    }
    ...
    if (!nconf.get('httpsPort')) {
        ...
        return q.ninvoke(httpServer, 'listen', nconf.get('httpPort')).then(function() {
