...
//resolve references in json specified by $ref and $id attribute, this is handy when user send cycles in json
    ...
    req.body.data = resolveReferences(req.body.data);
    ...
    var sandbox = {
        ...
        require: _require,
        render: require("./" + req.body.template.engine + "Engine" + ".js"),
    ...
    if (req.body.template.helpers) {
        ...
        vm.runInNewContext("jsrHelpers = " + req.body.template.helpers, sandbox);

        if (sandbox.jsrHelpers && typeof sandbox.jsrHelpers === 'object') {
            req.body.template.helpers = sandbox.jsrHelpers;
        } else {

            vm.runInNewContext(req.body.template.helpers, sandbox);

            req.body.template.helpers = {};
            for (var fn in sandbox) {
                if (typeof sandbox[fn] === "function") {
                    req.body.template.helpers[fn] = sandbox[fn];
                }
            }
        }
    } else
