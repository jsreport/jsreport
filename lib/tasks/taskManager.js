...
TaskManager.prototype.start = function () {
    ...
    return findFreePort().then(function (port) {
        ...
        process.env.numberOfWorkers = self.options.numberOfWorkers;
        process.env.port = self.options.port;
        self.workersCluster = childProcess.fork(path.join(__dirname, "workersCluster.js"), [], { env: process.env});
...
TaskManager.prototype.execute = function (options) {
    ...
    request({
        ...
        options.isDone = true;
        self._runningRequests = _.without(self._runningRequests, _.findWhere(self._runningRequests, {rid: self.options.rid}));
