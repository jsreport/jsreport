...
function addDomainCluster(app, server, cluster) {
...
ReportingServer.prototype._startServer = function (cluster) {
    var app = express();
    ...
    this._initReporter(app, function () {
        ...
        var server = https.createServer(credentials, app).listen(nconf.get('port'));

        if (nconf.get('useCluster')) {
            addDomainCluster(app,server, cluster);
        }
    });
};
