...
$C('$data.storageProviders.mongoDB.mongoDBProvider', $data.StorageProviderBase, null,
{
    ...
    _getDb: function(cb) {
        var self = this;
        if (dbs[this.providerConfiguration.databaseName]) {
            return cb(null, dbs[this.providerConfiguration.databaseName]);
        }

        var connectionString = "mongodb://";

        if (!_.isArray(this.providerConfiguration.address)) {
            this.providerConfiguration.address = [this.providerConfiguration.address];
            this.providerConfiguration.port = [this.providerConfiguration.port];
        }

        for (var i = 0; i < this.providerConfiguration.address.length; i++){
            connectionString += this.providerConfiguration.address[i] + ":" + this.providerConfiguration.port[i] + ",";
        }

        connectionString = connectionString.substring(0, connectionString.length - 1);
        connectionString += "/" + this.providerConfiguration.databaseName;

        if (this.providerConfiguration.replicaSet) {
            connectionString += "?replicaSet=" + this.providerConfiguration.replicaSet;
        }        
        ...
        mongodb.MongoClient.connect(connectionString , function(err, database) {
            ...
            dbs[self.providerConfiguration.databaseName] = database;
            cb(null, database);
        });
    },
    ...
    initializeStore: function(callBack){
        ...
        this._getDb(function(error, db){
            ...
            var fn = function(error, db){
                var cnt = 0;
                var collectionCount = 0;
                var readyFn = function(db, entitySet){
                    ...
                    if (entitySet){
                        ...
                        if (entitySetIndices && typeof self._createIndices === 'function'){
                            self._createIndices(db, entitySet, entitySetIndices, countFn);
                        }else countFn();
                    }else countFn();
                };
                ...
                if (!sets.length) return readyFn(db);
                sets.forEach(function(i){
                    if (self.context._entitySetReferences.hasOwnProperty(i)){
                        db.collectionNames({ namesOnly: true }, function(error, names){
                            ...
                            switch (self.providerConfiguration.dbCreation){
                                case $data.storageProviders.DbCreationType.DropAllExistingTables:
                                    if (names.indexOf(self.context._entitySetReferences[i].tableName) >= 0){
                                        db.dropCollection(self.context._entitySetReferences[i].tableName, function(error, result){
                                            if (error){
                                                callBack.error(error);
                                                return;
                                            }
                                            if (self.context._entitySetReferences[i].tableOptions){
                                                db.createCollection(self.context._entitySetReferences[i].tableName, self.context._entitySetReferences[i].tableOptions, function(error, result){
                                                    if (error){
                                                        callBack.error(error);
                                                        return;
                                                    }
                                                    readyFn(db, self.context._entitySetReferences[i]);
                                                });
                                            }else readyFn(db, self.context._entitySetReferences[i]);
                                        });
            ...
            if (self.providerConfiguration.username){
                db.authenticate(self.providerConfiguration.username, self.providerConfiguration.password || '', function(error, result){
                    if (error){
                        callBack.error(error);
                        return;
                    }
