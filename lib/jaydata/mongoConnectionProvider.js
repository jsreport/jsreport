...
module.exports = function(providerConfiguration) {
    logger = providerConfiguration.logger;
    dbName = providerConfiguration.databaseName;
        ...
        tryWithRetry(function (_cb) {
            ...
            process.nextTick(function () {
                _waitingRequests.forEach(function (req) {
                    req.cb(err, database.db(req.dbName));
                });
            });
    ...
    function tryWithRetry(fn, cb, retry) {
        ...
        fn(function (err, result) {
            ...
            retry++;
            logger.warn("Retry in once second");
            setTimeout(function () {
    ...
    function getDbWithoutRetry(dbName, cb) {
        ...
        d.on('error', function (er) {
            logger.warn("Mongo client running inside node domain crashed. Recycling domain... " + err)
            _cachedDb = null;
            cb(err);
        });
        ...
        d.run(function () {
            ...
            mongodb.MongoClient.connect(connectionString, { server: { auto_reconnect: true, socketOptions: socketOptions },
                ...
                if (err) {
                    logger.warm("initial connection to mongo failed " + err.stack);
                    return cb(err);
                }
