...
module.exports = async function resolveImageSrc (src, writeTempFileStream) {
  let imageContent
  let imageExtension
  let imageSource
  ...
  try {
    if (typeof src === 'string' && src.startsWith('data:')) {
      ...
      const { pathToFile: tmpImagePath, stream: tmpImageStream } = await writeTempFileStream((uuid) => `html-to-xlsx-rmt-img-${uuid}.${imageExtension}`)
