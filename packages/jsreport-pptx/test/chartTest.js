  ...
  it('chart', async () => {
    ...
    const result = await reporter.render({
      template: {
        ...
        pptx: {
          templateAsset: {
            content: fs.readFileSync(path.join(pptxDirPath, 'chart.pptx'))
          }
        }
      },
      data: {
    ...
    const [doc] = await getDocumentsFromPptxBuf(result.content, ['ppt/charts/chart1.xml'])
    const dataElements = nodeListToArray(doc.getElementsByTagName('c:ser'))

    dataElements.forEach((dataEl, idx) => {
      should(dataEl.getElementsByTagName('c:tx')[0].getElementsByTagName('c:v')[0].textContent).be.eql(datasets[idx].label)
      should(nodeListToArray(dataEl.getElementsByTagName('c:cat')[0].getElementsByTagName('c:v')).map((el) => el.textContent)).be.eql(labels)
      should(nodeListToArray(dataEl.getElementsByTagName('c:val')[0].getElementsByTagName('c:v')).map((el) => parseInt(el.textContent, 10))).be.eql(datasets[idx].data)
    })
  })
  ...
  it('chart should allow adding more data series than the ones defined in template', async () => {
    ...
    const result = await reporter.render({
      template: {
        ...
        pptx: {
          templateAsset: {
            content: fs.readFileSync(path.join(pptxDirPath, 'chart-data-series.pptx'))
          }
        }
      },
      data: {
  ...
  it('chart should allow axis configuration for display', async () => {
    ...
    const result = await reporter.render({
      template: {
        ...
        pptx: {
          templateAsset: {
            content: fs.readFileSync(path.join(pptxDirPath, 'chart-options-axis.pptx'))
          }
        }
      },
      data: {
  ...
  it('chart should allow setting datalabels', async () => {
    ...
    const result = await reporter.render({
      template: {
        ...
        pptx: {
          templateAsset: {
            content: fs.readFileSync(path.join(pptxDirPath, 'basic-scatter-chart-datalabels.pptx'))
          }
        }
      },
      data: {
  ...
  it('chart without style, color xml files', async () => {
    ...
    const result = await reporter.render({
      template: {
        ...
        pptx: {
          templateAsset: {
            content: fs.readFileSync(path.join(pptxDirPath, 'chart-with-no-style-colors-xml-files.pptx'))
          }
        }
      },
      data: {
  ...
  it('chart with title', async () => {
    ...
    const result = await reporter.render({
      template: {
        ...
        pptx: {
          templateAsset: {
            content: fs.readFileSync(path.join(pptxDirPath, 'chart-with-title.pptx'))
          }
        }
      },
      data: {
  ...
  it('scatter chart', async () => {
    ...
    const result = await reporter.render({
      template: {
        ...
        pptx: {
          templateAsset: {
            content: fs.readFileSync(path.join(pptxDirPath, 'scatter-chart.pptx'))
          }
        }
      },
      data: {
  ...
  it('bubble chart', async () => {
    ...
    const result = await reporter.render({
      template: {
        ...
        pptx: {
          templateAsset: {
            content: fs.readFileSync(path.join(pptxDirPath, 'bubble-chart.pptx'))
          }
        }
      },
      data: {
  ...
  it('stock chart', async () => {
    ...
    const result = await reporter.render({
      template: {
        ...
        pptx: {
          templateAsset: {
            content: fs.readFileSync(path.join(pptxDirPath, 'stock-chart.pptx'))
          }
        }
      },
      data: {
  ...
  it('combo chart (chart that uses a different chart type per data serie)', async () => {
    ...
    const result = await reporter.render({
      template: {
        ...
        pptx: {
          templateAsset: {
            content: fs.readFileSync(path.join(pptxDirPath, 'combo-chart.pptx'))
          }
        }
      },
      data: {
  ...
  it('waterfall chart (chartex)', async () => {
    ...
    const result = await reporter.render({
      template: {
        ...
        pptx: {
          templateAsset: {
            content: fs.readFileSync(path.join(pptxDirPath, 'waterfall-chart.pptx'))
          }
        }
      },
      data: {
    ...
    fs.writeFileSync(outputPath, result.content)
    const [doc] = await getDocumentsFromPptxBuf(result.content, ['ppt/charts/chartEx1.xml'])
  ...
  it('funnel chart (chartex)', async () => {
    ...
    const result = await reporter.render({
      template: {
        ...
        pptx: {
          templateAsset: {
            content: fs.readFileSync(path.join(pptxDirPath, 'funnel-chart.pptx'))
          }
        }
      },
      data: {
  ...
  it('treemap chart (chartex)', async () => {
    ...
    const result = await reporter.render({
      template: {
        ...
        pptx: {
          templateAsset: {
            content: fs.readFileSync(path.join(pptxDirPath, 'treemap-chart.pptx'))
          }
        }
      },
      data: {
  ...
  it('sunburst chart (chartex)', async () => {
    ...
    const result = await reporter.render({
      template: {
        ...
        pptx: {
          templateAsset: {
            content: fs.readFileSync(path.join(pptxDirPath, 'sunburst-chart.pptx'))
          }
        }
      },
      data: {
  ...
  it('clusteredColumn (chartex)', async () => {
    ...
    const result = await reporter.render({
      template: {
        ...
        pptx: {
          templateAsset: {
            content: fs.readFileSync(path.join(pptxDirPath, 'clusteredColumn-chart.pptx'))
          }
        }
      },
      data: {
    ...
    return reporter
      .render({
        template: {
          ...
          pptx: {
            templateAsset: {
              content: fs.readFileSync(path.join(pptxDirPath, 'chart-error-data.pptx'))
            }
          }
        },
        data: {
  ...
  it('chart loop', async () => {
    ...
    const result = await reporter.render({
      template: {
        ...
        pptx: {
          templateAsset: {
            content: fs.readFileSync(path.join(pptxDirPath, 'chart-loop.pptx'))
          }
        }
      },
      data: {
    ...
    const { files } = await getDocumentsFromPptxBuf(result.content, [], { returnFiles: true })
    const allSlideFiles = files.filter(f => f.path.includes('ppt/slides/slide'))

    for (const [slideIdx, slideFile] of allSlideFiles.entries()) {
      const slidePath = slideFile.path
      const slideDoc = new DOMParser().parseFromString(slideFile.data.toString())
      const slideNumber = parseInt(slideFile.path.replace('ppt/slides/slide', '').replace('.xml', ''))
      const slideRelsDoc = new DOMParser().parseFromString(files.find(f => f.path === `ppt/slides/_rels/slide${slideNumber}.xml.rels`).data.toString())
      const chartDrawingEls = nodeListToArray(slideDoc.getElementsByTagName('c:chart'))

      chartDrawingEls.length.should.be.eql(1)

      chartDrawingEls.forEach((chartDrawingEl) => {
        const chartRelId = chartDrawingEl.getAttribute('r:id')

        const chartRelEl = nodeListToArray(slideRelsDoc.getElementsByTagName('Relationship')).find((el) => {
          return el.getAttribute('Id') === chartRelId
        })

        const chartPath = path.posix.join(path.posix.dirname(slidePath), chartRelEl.getAttribute('Target'))

        const chartDoc = new DOMParser().parseFromString(
          files.find(f => f.path === chartPath).data.toString()
        )

        const chartName = path.posix.basename(chartPath)

        const chartRelsDoc = new DOMParser().parseFromString(
          files.find(f => f.path === `ppt/charts/_rels/${chartName}.rels`).data.toString()
        )

        const dataElements = nodeListToArray(chartDoc.getElementsByTagName('c:ser'))

        dataElements.forEach((dataEl, idx) => {
          dataEl.getElementsByTagName('c:tx')[0].getElementsByTagName('c:v')[0].textContent.should.be.eql(charts[slideIdx].chartData.datasets[idx].label)
          nodeListToArray(dataEl.getElementsByTagName('c:cat')[0].getElementsByTagName('c:v')).map((el) => el.textContent).should.be.eql(charts[slideIdx].chartData.labels)
          nodeListToArray(dataEl.getElementsByTagName('c:val')[0].getElementsByTagName('c:v')).map((el) => parseInt(el.textContent, 10)).should.be.eql(charts[slideIdx].chartData.datasets[idx].data)
        })
        ...
        const chartStyleDoc = files.find(f => f.path === `ppt/charts/${chartStyleRelEl.getAttribute('Target')}`)
        ...
        const chartColorStyleDoc = files.find(f => f.path === `ppt/charts/${chartColorStyleRelEl.getAttribute('Target')}`)
  ...
  it('chart loop and x, y axis titles', async () => {
    ...
    const result = await reporter.render({
      template: {
        ...
        pptx: {
          templateAsset: {
            content: fs.readFileSync(path.join(pptxDirPath, 'chart-loop-axis-titles.pptx'))
          }
        }
      },
      data: {
    ...
    for (const [slideIdx, slideFile] of allSlideFiles.entries()) {
      ...
      chartDrawingEls.forEach((chartDrawingEl) => {
        ...
        chartMainTitleEl.getElementsByTagName('a:t')[0].textContent.should.be.eql(`Fruit Chart ${(slideIdx + 1)}`)

        const chartCatAxTitleEl = chartTitles[1]

        chartCatAxTitleEl.getElementsByTagName('a:t')[0].textContent.should.be.eql(`X VALUE ${(slideIdx + 1)}`)

        const chartValAxTitleEl = chartTitles[2]

        chartValAxTitleEl.getElementsByTagName('a:t')[0].textContent.should.be.eql(`Y VALUE ${(slideIdx + 1)}`)
        ...
        dataElements.forEach((dataEl, idx) => {
          dataEl.getElementsByTagName('c:tx')[0].getElementsByTagName('c:v')[0].textContent.should.be.eql(chartList[slideIdx].chart_data.datasets[idx].label)
          nodeListToArray(dataEl.getElementsByTagName('c:cat')[0].getElementsByTagName('c:v')).map((el) => el.textContent).should.be.eql(chartList[slideIdx].chart_data.labels)
          nodeListToArray(dataEl.getElementsByTagName('c:val')[0].getElementsByTagName('c:v')).map((el) => parseInt(el.textContent, 10)).should.be.eql(chartList[slideIdx].chart_data.datasets[idx].data)
        })
  ...
  it('chart loop and x, y, secondary axis titles', async () => {
    ...
    const result = await reporter.render({
      template: {
        ...
        pptx: {
          templateAsset: {
            content: fs.readFileSync(path.join(pptxDirPath, 'chart-loop-secondary-axis-titles.pptx'))
          }
        }
      },
      data: {
    ...
    for (const [slideIdx, slideFile] of allSlideFiles.entries()) {
      ...
      chartDrawingEls.forEach((chartDrawingEl) => {
        ...
        chartAxTitleEl.getElementsByTagName('a:t')[0].textContent.should.be.eql(`X VALUE ${(slideIdx + 1)}`)

        const chartAxTitle2El = chartTitles[2]

        chartAxTitle2El.getElementsByTagName('a:t')[0].textContent.should.be.eql(`Y VALUE ${(slideIdx + 1)}`)

        const chartAxTitle3El = chartTitles[3]

        chartAxTitle3El.getElementsByTagName('a:t')[0].textContent.should.be.eql(`Y VALUE ${(slideIdx + 1)}`)

        const chartAxTitle4El = chartTitles[4]

        chartAxTitle4El.getElementsByTagName('a:t')[0].textContent.should.be.eql(`X VALUE ${(slideIdx + 1)}`)
  ...
  it('chart should keep style defined in serie', async () => {
    ...
    const result = await reporter.render({
      template: {
        ...
        pptx: {
          templateAsset: {
            content: fs.readFileSync(path.join(pptxDirPath, 'chart-serie-style.pptx'))
          }
        }
      },
      data: {
  ...
  it('chart should keep number format defined in serie', async () => {
    ...
    const result = await reporter.render({
      template: {
        ...
        pptx: {
          templateAsset: {
            content: fs.readFileSync(path.join(pptxDirPath, 'chart-serie-number-format.pptx'))
          }
        }
      },
      data: {
