...
module.exports = (doc, ext) => {
  return function processForm ({
    ...
    try {
      ...
      processType(formSpec, annotsObj, doc, position)
...
function processCheckbox (formSpec, annotation, doc, position) {
  ...
  if (doc._acroFormObj.properties.get('DR').get('Font').get('ZaDb') == null) {
    ...
    doc._acroFormObj.properties.get('DR').get('Font').set('ZaDb', zapfFontObject.toReference())
  }

  const zapfFontObject = doc._acroFormObj.properties.get('DR').get('Font').get('ZaDb').object
